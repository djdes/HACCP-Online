...
| 2026-04-26 | _PENDING_ | Демо-данные для ROOT — `POST /api/root/seed-demo-org` создаёт за 30 сек полную trial-организацию: positions + journals + 10-20 demo-сотрудников + JournalDocument на месяц + 7 дней history. Кнопка «Создать демо-ресторан» на `/root` |
