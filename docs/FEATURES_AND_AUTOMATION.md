...
| 2026-04-27 | (next commit) | Per-org metrics dashboard на `/root/metrics` — `getAllOrgMetrics()` собирает activeUsers, entries 7d/30d, weekly trend, last activity, potential/actual MRR (по `calculatePerEmployeePrice`). Сводные карточки сверху + таблица отсортированная по MRR. Кнопка «Метрики платформы» добавлена на `/root` |
