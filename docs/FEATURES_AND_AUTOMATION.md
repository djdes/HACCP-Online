...
| 2026-04-26 | `7bf3088`-`pending` | **EPIC: Shared journal tasks (event-log) + кнопка «Не требуется сегодня».** Большая переделка интеграции TF: 35 шаблонов классифицированы по `taskScope: personal \| shared`. Shared (acceptance, finished_product, complaint_register, accident_journal и 16 других event-журналов) теперь работают как открытая очередь записей: сотрудник может N раз нажать «Добавить запись», или «Не требуется сегодня» с выбором причины из настроенного списка. Compliance признаёт closure ✅. Реализовано в 7 этапов: schema (`7bf3088`) → backend API close/reopen/status (`90e3b88`) → классификация шаблонов + migration cron (`1792ee8`) → settings UI per-template `/settings/journals/[code]/scope` (`c4ab75f`) → compliance recognizes close-events (`c001196`) → UI на task-fill page с 3 кнопками (`4bba603`) → auto-close cron в `shiftEndHour` (pending). |
