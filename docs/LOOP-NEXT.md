...
- **[2026-05-08 17:25 МСК]** `34ac55d7` — **Cleaning UX unification: один редактор помещения + урезанный race-режим**.
  - Раньше владелец редактировал помещение в `/settings/buildings` через один диалог с полным набором полей (тип, средство, объёмы, расписание, фото-policy), а в журнале уборки — через урезанный inline `<Dialog>` с другим набором полей. Это было путающе.
  - Теперь и в журнале, и в настройках открывается один и тот же `<RoomEditorDialog>`. Все правки идут через `PATCH /api/settings/rooms/[id]`. Один источник правды, один UI.
  - Карандашик в строке таблицы стал кликабельным (раньше работало только нажатие на название).
  - `RoomsModeCard` (race-режим) урезан до on/off + выбор комнат + уборщиков. Поля «Контролёр (общий)» и «Контролёры по комнатам (override)» удалены — они дублировали то, что уже настраивается в `/settings/journal-responsibles`. Вместо них — banner-подсказка со ссылкой.
  - Backward-compat: `controlUserId` / `verifierByRoomId` остались опциональными в `CleaningDocumentConfig` и в адаптере (старые документы продолжат работать). Новые документы будут падать на `doc.verifierUserId` из `/settings/journal-responsibles`.
  - **Что заметил:** удалили ~230 строк UI-кода (включая `submitRoom` хендлер и весь inline `<Dialog>` block) — просто перейдя на shared-component. ROI как в `b4f678b6` (StaffJournalToolbar): одна правка покрывает два места и убивает дрейф.
  - **Что предлагаю дальше:** обкатать на проде → если уборщица/менеджер не жалуются, удалить опциональные поля `controlUserId`/`verifierByRoomId` из схемы (миграция данных + удаление из адаптера) — но это после нескольких дней наблюдения, не сейчас.
...
- [2026-05-08] `34ac55d7`: «два редактора одной сущности» — anti-pattern. Если та же сущность редактируется в двух разных контекстах (settings vs внутри workflow), оба контекста ОБЯЗАНЫ открывать один и тот же компонент диалога. Любое расхождение полей превращается в скрытый desync (юзер сохраняет в одном — другой не отображает). Решение: shared-component + единый API endpoint. Удалено 230 строк inline-формы. Принцип переносится на любые «edit-я-в-двух-местах» сценарии (например, юзеры на /settings/users vs in-context user editor — должны быть один и тот же компонент).
- [2026-05-08] `34ac55d7`: при удалении настроек из UI ВСЕГДА оставлять backward-compat в схеме/адаптере. Удалили `controlUserId`/`verifierByRoomId` из `RoomsModeCard.onSave` патча, но оставили optional поля в `CleaningDocumentConfig` и в `tasksflow-adapters/cleaning.ts` (с fallback'ом на `doc.verifierUserId`). Старые документы продолжают работать без миграции данных. После нескольких дней наблюдений в проде можно почистить, но не сразу.
