...
Деплой третьего круга: `a827bab2` (фича), `80c43b5c` (SHA заметок). Прод после `80c43b5c`: `/journals/hygiene/` → 308 (proxy работает), консультант «просмотр» — 403 на создание точки, документа и организации; сводка по точкам на дашборде, бейдж «Общий», блок «Сотрудники точки»; второй запрос `/api/mini/home` — 24 мс. Тестовые точки и партнёр удалены.
