...
## Круг 3 (2026-09-05, «реализуй всё кроме геолокации»)

Сделано: сводка по точкам на дашборде (клик переключает точку); бейдж «Общий» у документов без точки в 31 списочном клиенте; шаг «Назовите точки» после анкеты (`/api/profile/complete` возвращает созданные точки); точка в отчётах (heatmap/тренд/счётчики по документам активной точки), экспорте (`buildings.json`), инспекторской ссылке и PDF инспектора, сводном ZIP; «закрыть день без событий» и превью журналов по точкам (`buildingKey` в уникальных ключах `JournalCloseEvent`/`JournalPreview`, миграция применена на прод через `prisma db push --accept-data-loss` до деплоя); КПП и телефон точки в карточке и в шапке PDF; «Сотрудники точки» в карточке точки (`PUT /api/settings/buildings/[id]/staff`); главная Mini App — сверка обязательств в фоне (`obligation-sync-throttle`): второй запрос `/api/mini/home` ~100 мс; `PageHeader` переносит действия под заголовок; корневой `middleware.ts` (только no-cache) перекрывал `src/middleware.ts` в dev — удалён, логика перенесена в `src/proxy.ts` (конвенция Next 16); на dev guard партнёра «просмотр» теперь даёт 403.

`e2e/verify3.ts`, `api-check3.ts`, `naming-check.ts` (dev 3020): proxy 308 на трейлинг-слеш, партнёр 403 на создание точки/документа и на тумблер, сводка по точкам и переключение, 2 бейджа «Общий», КПП в карточке, «здесь: 1 · везде: 6» после назначения, подпись точки в отчётах, ответ `/api/profile/complete` с двумя точками, шаг «Назовите точки» на 390×780.
