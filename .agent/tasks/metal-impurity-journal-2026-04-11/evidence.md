...
- AC1: PASS. `metal_impurity` is now classified as a document template in the shared helper, so `/journals/[code]` and related routes render the screenshot-based document flow instead of the generic journal-entry flow; the page also auto-seeds active + closed demo documents for first-open parity.
...
- AC3: PASS. Document detail structure remains in the dedicated `MetalImpurityDocumentClient`, including the corrected two-row printable HACCP header, action row, table, settings, finish flow, and read-only closed state.
...
- AC12: PASS. Fresh `npm run build` succeeded on the current codebase after the routing-classification and printable-header fixes.
