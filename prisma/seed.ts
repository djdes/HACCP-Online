...
const additionalJournalTemplates = [
  ...
  // 5 новых шаблонов по методичке gosuslugi.pnzreg.ru/upload/FRGU/c8d/hassp_1_.pdf.
  // Закрывают §8.3, §8.4, §8.10 + примеры §2.3, §2.9.
  // См. docs/audit/2026-05-08-haccp-methodology-mapping.md.
  {
    code: "foreign_objects_briefing",
    name: "Журнал инструктажа по предотвращению попадания посторонних предметов",
    description:
      "Журнал регистрации инструктажей по инструкции о предотвращении попадания посторонних предметов в продукцию (§8.3 методички ХАССП).",
    sortOrder: 42,
    isMandatorySanpin: false,
    isMandatoryHaccp: true,
    fields: [
      { key: "briefingDate", label: "Дата инструктажа", type: "date", required: true },
      {
        key: "briefingType",
        ...
        required: true,
        options: [
          { value: "primary", label: "Первичный (при приёме на работу)" },
          { value: "scheduled", label: "Плановый (повторный)" },
          { value: "targeted", label: "Целевой (после нарушения)" },
          { value: "unscheduled", label: "Внеплановый (по событию)" },
        ],
      },
      { key: "instructor", label: "ФИО инструктора", type: "employee", required: true },
      { key: "attendees", label: "Список присутствовавших (ФИО, должности)", type: "textarea", required: true },
      {
        key: "testResult",
        ...
        required: true,
        options: [
          { value: "passed", label: "Усвоил" },
          { value: "failed", label: "Не усвоил — требуется повторный" },
        ],
      },
      { key: "correctiveAction", label: "Корректирующее действие", type: "text", required: false, showIf: { field: "testResult", equals: "failed" } },
      { key: "notes", label: "Примечание", type: "textarea", required: false },
    ],
  },
  {
    code: "haccp_contracts",
    name: "Реестр договоров ХАССП",
    description:
      "Реестр договоров с подрядчиками по обязательным мероприятиям (дератизация, дезинсекция, стирка сан.одежды, вывоз отходов, утилизация ртутных ламп). §8.4 методички ХАССП.",
    sortOrder: 43,
    isMandatorySanpin: true,
    isMandatoryHaccp: true,
    fields: [
      {
        ...
        options: [
          { value: "deratization", label: "Дератизация (грызуны)" },
          { value: "disinsection", label: "Дезинсекция (насекомые)" },
          { value: "sanitary_laundry", label: "Стирка санитарной одежды" },
          { value: "waste_removal", label: "Вывоз отходов" },
          { value: "lamp_disposal", label: "Утилизация ртутных/УФ-ламп" },
          { value: "ventilation_service", label: "Обслуживание вентиляции" },
          { value: "medical_exam", label: "Медицинские осмотры" },
          { value: "other", label: "Прочее" },
        ],
      },
      { key: "contractor", label: "Подрядчик (организация)", type: "text", required: true },
      { key: "contractNumber", label: "Номер договора", type: "text", required: true },
      { key: "contractDate", label: "Дата заключения", type: "date", required: true },
      { key: "expiryDate", label: "Срок действия до", type: "date", required: true },
      { key: "monthlyFee", label: "Стоимость (₽/месяц)", type: "number", required: false, step: 0.01 },
      { key: "contactPerson", label: "Контактное лицо подрядчика", type: "text", required: false },
      { key: "contactPhone", label: "Телефон подрядчика", type: "text", required: false },
      { key: "attachmentUrl", label: "Скан договора (URL)", type: "text", required: false },
      {
        key: "status",
        label: "Статус",
        type: "select",
        ...
        options: [
          ...
          { value: "terminated", label: "Расторгнут досрочно" },
        ],
      },
      { key: "notes", label: "Примечание", type: "textarea", required: false },
    ],
  },
  {
    ...
    description:
      "Журнал учёта документов, подтверждающих безопасность изготовленной продукции (декларации соответствия, сертификаты, ветеринарные свидетельства, СГР). §8.10 методички ХАССП.",
    sortOrder: 44,
    isMandatorySanpin: true,
    isMandatoryHaccp: true,
    fields: [
      { key: "productName", label: "Наименование продукции", type: "text", required: true },
      {
        ...
        options: [
          ...
          { value: "veterinary", label: "Ветеринарное свидетельство" },
          { value: "sgr", label: "СГР (свидетельство о государственной регистрации)" },
          { value: "quality_certificate", label: "Качественное удостоверение" },
          { value: "other", label: "Прочее" },
        ],
      },
      { key: "documentNumber", label: "Номер документа", type: "text", required: true },
      { key: "issueDate", label: "Дата выдачи", type: "date", required: true },
      { key: "expiryDate", label: "Срок действия до", type: "date", required: true },
      { key: "issuingAuthority", label: "Кем выдан", type: "text", required: true },
      { key: "batchNumber", label: "Партия (если применимо)", type: "text", required: false },
      { key: "attachmentUrl", label: "Скан (URL)", type: "text", required: false },
      {
        key: "status",
        label: "Статус",
        type: "select",
  ...
  },
  {
    ...
    description:
      "Журнал визуальной оценки санитарного состояния производственных объектов (отделений, участков, цехов) с регистрацией корректирующих мероприятий. §2.3 методички ХАССП (стр. 27).",
    sortOrder: 45,
    isMandatorySanpin: true,
    isMandatoryHaccp: true,
    fields: [
      { key: "inspectionDate", label: "Дата проверки", type: "date", required: true },
      {
        key: "inspectionObject",
        label: "Объект (отделение / участок / цех)",
        type: "select",
        required: true,
        options: [
          { value: "hot_shop", label: "Горячий цех" },
          { value: "cold_shop", label: "Холодный цех" },
          { value: "meat_shop", label: "Мясной цех" },
          { value: "fish_shop", label: "Рыбный цех" },
          { value: "vegetable_shop", label: "Овощной цех" },
          { value: "confectionery", label: "Кондитерский цех" },
          { value: "dishwashing", label: "Моечная посуды" },
          { value: "warehouse", label: "Склад" },
          { value: "cold_storage", label: "Холодильная камера" },
          { value: "trade_hall", label: "Торговый зал" },
          { value: "other", label: "Прочее (указать в примечании)" },
        ],
      },
      {
        key: "sanitaryAssessment",
        label: "Оценка санитарного состояния",
        type: "select",
        required: true,
        options: [
          { value: "excellent", label: "Отлично — нарушений нет" },
          { value: "good", label: "Хорошо — мелкие замечания" },
          { value: "satisfactory", label: "Удовлетворительно — требует исправления" },
          { value: "unsatisfactory", label: "Неудовлетворительно — недопустимое состояние" },
        ],
      },
      { key: "correctiveAction", label: "Корректирующие мероприятия", type: "textarea", required: false, showIf: { field: "sanitaryAssessment", equals: "unsatisfactory" } },
      { key: "actionTaken", label: "Отметка о принятых мерах", type: "textarea", required: false },
      { key: "responsibleName", label: "Ответственное лицо", type: "employee", required: true },
      { key: "notes", label: "Примечание", type: "textarea", required: false },
    ],
  },
  {
    ...
    description:
      "Журнал планирования и регистрации корректирующих действий по выявленным несоответствиям ХАССП. §2.9 методички ХАССП (стр. 56).",
    sortOrder: 46,
    isMandatorySanpin: false,
    isMandatoryHaccp: true,
    fields: [
      { key: "detectionDate", label: "Дата обнаружения", type: "date", required: true },
      { key: "detectionLocation", label: "Место обнаружения", type: "text", required: true },
      { key: "nonconformity", label: "Описание несоответствия", type: "textarea", required: true },
      {
        key: "severity",
        label: "Серьёзность",
        type: "select",
        required: true,
        options: [
          { value: "low", label: "Низкая (предупреждение)" },
          { value: "medium", label: "Средняя (требует устранения в течение недели)" },
          { value: "high", label: "Высокая (угроза безопасности продукции)" },
        ],
      },
      { key: "correctiveAction", label: "Корректирующее действие", type: "textarea", required: true },
      { key: "responsibleExecution", label: "Ответственный за выполнение", type: "employee", required: true },
      { key: "plannedExecutionDate", label: "Плановая дата выполнения", type: "date", required: true },
      { key: "actualExecutionDate", label: "Фактическая дата выполнения", type: "date", required: false },
      { key: "responsibleControl", label: "Ответственный за контроль", type: "employee", required: true },
      { key: "controlDate", label: "Дата контроля", type: "date", required: false },
      {
        key: "result",
        label: "Результат",
        type: "select",
        required: false,
        options: [
          { value: "resolved", label: "Устранено полностью" },
          { value: "partial", label: "Устранено частично" },
          { value: "unresolved", label: "Не устранено — требуются доп. меры" },
        ],
      },
      {
        key: "sourceJournalCode",
        label: "Источник (код журнала, если связан)",
        type: "text",
        required: false,
      },
    ],
