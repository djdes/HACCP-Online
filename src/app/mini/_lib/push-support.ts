...
export function pushState(env: PushEnvironment): PushState {
  // iOS проверяем ПЕРВЫМ, до `supported`, и это не придирка к порядку.
  // Safari отдаёт `window.PushManager` только установленному
  // приложению: в обычной вкладке его нет вовсе, то есть `supported`
  // там всегда false. Проверяй мы поддержку раньше, на айфоне мы
  // молча показывали бы пустоту — ровно там, где человеку и нужно
  // объяснить, что приложение надо добавить на экран «Домой».
  //
  // Отсутствие Push API во вкладке iOS — не «браузер не умеет», а
  // «ещё не установлено», и лечится это установкой.
