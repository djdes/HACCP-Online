            ...
            <div>
              <h2 className="text-[20px] font-semibold text-[#0b1024]">
                {complianceItems.length === 0
                  ...
                  : unfilledCount === 0
                    ? "Все ежедневные журналы на сегодня заполнены"
                    : `Нужно заполнить сегодня: ${unfilledCount}`}
              </h2>
              <p className="mt-0.5 text-[13px] text-[#6f7282]">
                {complianceItems.length === 0
                  ? "Ежедневные журналы (гигиена, температура и т.п.) пока не настроены."
                  : unfilledCount === 0
                    ? "Отличная работа. Журналы по событию (аварии, жалобы, аудиты) заполняются по факту."
                    : "Нажмите на карточку, чтобы открыть журнал и внести запись за сегодняшнее число."}
              </p>
            </div>
          </div>
