...
/**
 * Полоса автозаполнения. Высота эталона — ~48px (H5 аудита): штатный
 * тумблер 44×24 + 12px паддинга сверху и снизу. Прежние `py-3.5 sm:py-4`
 * вместе с увеличенным тумблером давали ~62px.
 */
export const DOC_AUTOFILL_STRIP_CLASS =
  "mb-10 rounded-[22px] bg-[#f3f4fe] px-4 py-3 print:hidden sm:px-6";
...
/**
 * «Бумажное полотно» документа — S8 аудита.
 *
 * Эталон lk.haccp-online.ru рисует бланк (шапка ХАССП → КАПС-титул →
 * «Добавить» → таблица → пояснения/легенда) НЕ во всю ширину контентной
 * колонки (1296px), а центрированным блоком ~1150px с полями по бокам.
 * Крошки, H1 и полоса автозаполнения при этом остаются во всю ширину —
 * поэтому токен вешается на обёртку ИМЕННО бумажной зоны, а не на
 * контейнер страницы.
 *
 * Таблицы шире 1150px (cold_equipment на 15 дней, приёмка на 11 колонок,
 * уборка на 1200) продолжают скроллиться ВНУТРИ своего
 * `GRID_VIEWPORT_CLASS`, который лежит внутри полотна.
 *
 * `print:max-w-none` — на бумаге ограничение снимается, лист и так
 * задаётся `@page`.
 */
export const DOC_PAPER_CANVAS_CLASS =
  "mx-auto w-full max-w-[1150px] print:max-w-none";
