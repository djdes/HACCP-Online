...
export function getDefaultAuditReportConfig(): AuditReportConfig {
  ...
  return {
    ...
    auditors: [],
    // ПОЧЕМУ пусто: выводы, рекомендации и несоответствия — это итог
    // реального аудита. Предзаполненный отчёт о «несвоевременно
    // заполненных записях ККТ» в новом документе — выдуманный факт.
    summary: "",
    recommendations: "",
    findings: [],
    signatures: [
