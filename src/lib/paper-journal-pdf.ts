...
export function renderPaperJournalPdf(params: {
  ...
  // «Электронная форма не принимается» верно только для инструктажей по
  // охране труда (ТК РФ ст. 22.1). Печатать это на пожарном журнале,
  // который закон разрешает вести электронно, — вводить в заблуждение
  // на бумаге, которую положат в папку для инспектора.
  doc.text(
    journal.paperOnly
      ? `Бланк для бумажного ведения. Электронная форма не принимается — ${journal.law.label}. Штраф ${journal.fineHint}.`
      : `Бланк для бумажного ведения — ${journal.law.label}. Штраф ${journal.fineHint}.`,
    pageWidth / 2,
    36,
    { align: "center" },
  );
